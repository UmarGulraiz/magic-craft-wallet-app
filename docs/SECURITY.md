# Magiccraft Web Extension Wallet - Security Guidelines

At Magiccraft, we prioritize the security and privacy of our users' digital assets and personal information. This document outlines the security measures and features we have implemented to ensure a secure web3 wallet experience.
...
Magiccraft uses a permission system that restricts what websites and applications can access to the account addresses, ensuring that user data and actions remain secure.

### 10. Session Timeout

Magiccraft implements session timeouts to automatically log out users after a period of inactivity, reducing the risk of unauthorized access.
...
- To address potential security vulnerabilities, Magiccraft keeps the app updated with the latest security patches and enhancements.
...
- Magiccraft implements anti-phishing protection to protect users about suspicious websites. Powered by [eth-phishing-detect](https://github.com/MetaMask/eth-phishing-detect).

- Magiccraft uses unique profile avatars. This feature allows you to determine whether the Magiccraft you are seeing is real or fake.
...
Magiccraft collects and stores only the minimum amount of user data necessary for the operation of the application UI, respecting user privacy.

### 16. Open Source Transparency

Magiccraft is committed to transparency and is open source, allowing the community to review and contribute to security improvements.
...
Magiccraft offers responsive customer support to assist users in case of security concerns or issues.

### 19. Regular Penetration Testing

Magiccraft conducts regular penetration testing to proactively identify vulnerabilities before malicious actors can exploit them.

Security is an ongoing process, and we remain committed to continuously monitoring and updating Magiccraft to stay ahead of potential risks and protect our users' assets and data.
